...
public class JdbcToDoRepository implements ToDoRepository {
    ...
    public void save(ToDo todo) {
        String sql = "INSERT INTO TODOS (username, todo_date, title, content, done) VALUES (?, ?, ?, ?, ?)";
    ...
    public void delete(String username, LocalDate date) {
        String sql = "DELETE FROM TODOS WHERE username = ? AND todo_date = ?";
    ...
    public void markAsDone(String username, LocalDate date) {
        String sql = "UPDATE TODOS SET done = 'Y' WHERE username = ? AND todo_date = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, username);
            pstmt.setDate(2, Date.valueOf(date));
            pstmt.executeUpdate();
